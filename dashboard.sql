...
SELECT
    ...
    ROUND(
        AVG(EXTRACT(EPOCH FROM (l.created_at - s.visit_date)) / 86400)::numeric,
        2
    )
        AS avg_days_to_lead,
    ROUND(PERCENTILE_CONT(0.9) WITHIN GROUP (
        ORDER BY EXTRACT(EPOCH FROM (l.created_at - s.visit_date)) / 86400
    )::numeric, 2) AS percentile_90_days
FROM sessions AS s
INNER JOIN leads AS l ON s.visitor_id = l.visitor_id
WHERE
    (l.closing_reason = 'Успешно реализовано' OR l.status_id = 142)
    AND s.visit_date <= l.created_at
GROUP BY s.source, s.campaign;

--Детальный анализ эффективности рекламных кампаний по источникам, 
--каналам и конкретным кампаниям за июнь 2023 года.
WITH last_paid_click AS (
    ...
    LEFT JOIN
        ...
        AND s.visit_date <= l.created_at
    WHERE
        s.medium IN ('cpc', 'cpm', 'cpa', 'youtube',
        'cpp', 'tg', 'social')
),
...
NULLIF(SUM(total_cost), 0) * 100, 2
  ) AS roi,
  ROUND(SUM(total_cost) / NULLIF(SUM(purchases_count),
0), 2) AS cppu
FROM marketing_data
...
--Сводный анализ эффективности рекламных каналов по источникам трафика за июнь 2023 года.
WITH last_paid_click AS (
    SELECT
        s.visitor_id,
        s.source AS utm_source,
...
SELECT
  ...
  SUM(leads_count) AS leads,
  SUM(purchases_count) AS purchases,
  ROUND(SUM(total_cost) / NULLIF(SUM(visitors_count),
  0), 2) AS cpu,
  ROUND(SUM(total_cost) / NULLIF(SUM(leads_count), 0),
  2) AS cpl,
  ROUND(SUM(total_cost) / NULLIF(SUM(purchases_count),
  0), 2) AS cppu,
  ROUND(
...
--Конверсионная воронка по источникам трафика за июнь 2023 года.
WITH last_paid_click AS (
    SELECT
        s.visitor_id,
        s.source AS utm_source,
...
SELECT
    ...
    SUM(leads_count) AS leads,
    SUM(purchases_count) AS purchases,
    ROUND(SUM(leads_count) * 100.0 /
NULLIF(SUM(visitors_count), 0), 2)
        AS visit_to_lead_conv,
    ROUND(SUM(purchases_count) * 100.0 /
NULLIF(SUM(leads_count), 0), 2)
