...
SELECT
    ...
    ROUND(
        SUM(purchases_count) * 100.0 / NULLIF(SUM(visitors_count), 0), 2
    ) AS overall_conv
FROM
    marketing_data
WHERE
...
SELECT
    ...
    ROUND(
        SUM(total_cost) / NULLIF(SUM(purchases_count), 0), 2
    ) AS cppu
FROM
    marketing_data
WHERE
...
SELECT
    ...
    SUM(purchases_count) AS purchases,
    ROUND(
        SUM(total_cost) / NULLIF(SUM(visitors_count), 0), 2
    ) AS cpu,
    ROUND(
        SUM(total_cost) / NULLIF(SUM(leads_count), 0), 2
    ) AS cpl,
    ROUND(
        SUM(total_cost) / NULLIF(SUM(purchases_count), 0), 2
    ) AS cppu,
