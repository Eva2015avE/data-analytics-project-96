...
SELECT
    ...
    ROUND(
        AVG(EXTRACT(EPOCH FROM (l.created_at -
s.visit_date)) / 86400)::numeric,
        2
    ) AS avg_days_to_lead,
    ROUND(
        PERCENTILE_CONT(0.9) WITHIN GROUP (
            ORDER BY EXTRACT(EPOCH FROM (l.created_at -
s.visit_date)) / 86400
        )::numeric,
        2
...
WHERE
    (l.closing_reason = 'Успешно реализовано' OR
l.status_id = 142)
